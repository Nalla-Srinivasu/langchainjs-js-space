...
const structureModel =  model.withStructuredOutput(CompanySchema,{strict:true})

const template = ChatPromptTemplate.fromMessages([
    ["system","Extract company information from  the text . If information is not availble, make reasonable estimates"],
    ["human","{text}"]
]);

const chain = template.pipe(structureModel);

const companyInfo = `Microsoft was founded in 1975 and is headquartered in Redmond, Washington. The company is publicly traded and has over 220,000 employees worldwide.Their main products include Windows, Office, Azure, and Xbox.`
const result = await chain.invoke({text: companyInfo})

console.log("Extract the company data")
